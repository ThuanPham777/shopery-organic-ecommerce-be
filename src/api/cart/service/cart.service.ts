...
export class CartService {
  ...
  async getCart(userId?: number, sessionId?: string): Promise<Cart> {
    if (userId) {
      return this.getUserCart(userId); // Lấy giỏ hàng theo user (database)
    }

    if (sessionId) {
      return this.getGuestCart(sessionId); // Lấy giỏ hàng theo sessionId (Redis cho khách)
    }
  ...
  private async addToGuestCart(sessionId: string, addCartItemDto: AddCartItemInDto): Promise<Cart> {
    ...
    // Nếu sản phẩm đã tồn tại trong giỏ hàng, cộng số lượng
